...
import { BASE_URL, BASE_IMAGE_URL } from '@/utils/api';
...
const CategoriesPage = () => {
  ...
  const openEditModal = async (category) => {
    const res = await fetch(`${BASE_URL}/get_one_category/${category.id}`);
  ...
  const handleSubmit = (e) => {
    ...
    const url =
      modalType === 'add'
        ...
        : `${BASE_URL}/update_one_category/${formData.id}`;
  ...
  const handleDelete = (id) => {
    ...
    fetch(`${BASE_URL}/delete_one_category/${id}`, {
      method: 'DELETE',
    })
      .then((res) => res.json())
  ...
  return (
    <div className="p-10">
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-4xl font-bold text-red-700">الأصناف</h1>
        <button
      ...
      {isModalOpen && (
        <Modal
          ...
          title={modalType === 'add' ? 'إضافة صنف جديد' : 'تعديل الصنف'}
          fields={[{ name: 'name', label: 'اسم الصنف' }]}
          formData={formData}
          setFormData={setFormData}
        >
          <div className="mb-4">
            <label className="block mb-1 font-semibold">صورة الصنف</label>
            <input
              type="file"
              accept="image/*"
